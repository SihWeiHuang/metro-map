...
import { resolveLineDisplayNameFromProps } from "./defaultNames.js";
 ...
 */
export function showRouteHoverPopup(lngLat, routeId, point, data) {
  if (!canShowRouteHoverPopup()) return;

  const map = mapInstance();
  if (!map) return;

  const currentRoute = data.routes.find((x) => x.properties.route_id === routeId);
  if (!currentRoute) return;

  const lineId = currentRoute.properties.group_id;
  const headRoute = data.routes.find((f) => f.properties.group_id === lineId);
